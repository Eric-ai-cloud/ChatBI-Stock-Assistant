"""
测试：验证智能SQL预处理功能
检测并修正"最近一个月"查询（从30个交易日转换为自然月30天）
"""
import pandas as pd
from sqlalchemy import create_engine, text
import os
import re
from datetime import datetime, timedelta

def preprocess_recent_month_sql(sql_input: str) -> str:
    """
    智能预处理SQL：检测并修正"最近一个月"查询
    
    问题：LLM可能生成 LIMIT 30（30个交易日≈43天）
    解决：转换为自然月30天的查询
    
    返回：修正后的SQL
    """
    
    # 检测是否包含 "ORDER BY trade_date DESC LIMIT 30" 模式
    pattern = r'ORDER\s+BY\s+trade_date\s+DESC\s+LIMIT\s+30'
    
    if not re.search(pattern, sql_input, re.IGNORECASE):
        # 不是"最近一个月"的查询，直接返回
        return sql_input
    
    print("⚠️ 检测到'最近一个月'查询（LIMIT 30），正在转换为自然月30天...")
    
    # 提取股票代码
    stock_code_match = re.search(r"stock_code\s*=\s*['\"]([^'\"]+)['\"]", sql_input)
    if not stock_code_match:
        print("⚠️ 未找到股票代码，无法转换")
        return sql_input
    
    stock_code = stock_code_match.group(1)
    
    # 获取最新交易日期
    db_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'stock_prices.db')
    engine = create_engine(f'sqlite:///{db_path}')
    
    try:
        latest_date_sql = f"SELECT MAX(trade_date) as max_date FROM stock_history WHERE stock_code = '{stock_code}'"
        df_latest = pd.read_sql(text(latest_date_sql), engine)
        latest_date_str = df_latest['max_date'].iloc[0]
        
        if not latest_date_str:
             print("⚠️ 未找到最新交易日期，无法转换")
             return sql_input
        
        # 计算30天前的日期
        latest_date = datetime.strptime(str(latest_date_str), '%Y%m%d')
        start_date = latest_date - timedelta(days=30)
        start_date_str = start_date.strftime('%Y%m%d')
        
        print(f"   最新日期: {latest_date_str}")
        print(f"   起始日期: {start_date_str} (往前推30天)")
        
        # 替换SQL：将子查询 + LIMIT 30 替换为具体日期范围
        # 匹配模式：FROM ( SELECT ... ORDER BY trade_date DESC LIMIT 30) ORDER BY trade_date ASC
        subquery_pattern = r'FROM\s*\(\s*SELECT\s+(.*?)\s+FROM\s+stock_history\s+WHERE\s+stock_code\s*=\s*[\'"][^\'"]+[\'"]\s+ORDER\s+BY\s+trade_date\s+DESC\s+LIMIT\s+30\s*\)\s*ORDER\s+BY\s+trade_date\s+ASC'
        
        match = re.search(subquery_pattern, sql_input, re.IGNORECASE | re.DOTALL)
        if match:
            # 提取SELECT字段
            select_fields = match.group(1).strip()
            
            # 构建新的SQL
            new_sql = f"""SELECT {select_fields} 
FROM stock_history 
WHERE stock_code = '{stock_code}' 
  AND trade_date >= '{start_date_str}'
ORDER BY trade_date ASC"""
            
            print(f"   ✅ SQL已修正为自然月30天查询")
            return new_sql
        else:
            # 如果没有匹配到子查询模式，尝试简单替换
            print("   ⚠️ 未匹配到标准子查询模式，使用备选方案")
            
            # 直接在WHERE中添加日期条件
            if 'WHERE' in sql_input.upper():
                new_sql = re.sub(
                    r'(WHERE\s+stock_code\s*=\s*[\'"][^\'"]+[\'"])',
                    f"\\1 AND trade_date >= '{start_date_str}'",
                    sql_input,
                    flags=re.IGNORECASE
                )
                # 移除 LIMIT 30
                new_sql = re.sub(r'\s+LIMIT\s+30', '', new_sql, flags=re.IGNORECASE)
                return new_sql
            
    except Exception as e:
        print(f"   ❌ SQL预处理失败: {str(e)}")
        # 如果处理失败，返回原始SQL
        return sql_input
    
    return sql_input


def test_sql_preprocessing():
    """测试SQL预处理功能"""
    
    print("="*70)
    print("测试：智能SQL预处理功能")
    print("="*70)
    
    # 测试用例1：标准的子查询模式
    sql1 = """SELECT * FROM (
        SELECT trade_date, close_price 
        FROM stock_history 
        WHERE stock_code = '600519.SH' 
        ORDER BY trade_date DESC 
        LIMIT 30
    ) ORDER BY trade_date ASC"""
    
    print("\n📝 测试用例1：标准子查询模式")
    print("原始SQL:")
    print(sql1)
    
    processed_sql1 = preprocess_recent_month_sql(sql1)
    print("\n处理后SQL:")
    print(processed_sql1)
    
    # 执行处理后的SQL
    db_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'stock_prices.db')
    engine = create_engine(f'sqlite:///{db_path}')
    
    df1 = pd.read_sql(text(processed_sql1), engine)
    print(f"\n✅ 返回 {len(df1)} 条记录")
    if len(df1) > 0:
        print(f"日期范围：{df1['trade_date'].iloc[0]} 至 {df1['trade_date'].iloc[-1]}")
        
        start_date = datetime.strptime(str(df1['trade_date'].iloc[0]), '%Y%m%d')
        end_date = datetime.strptime(str(df1['trade_date'].iloc[-1]), '%Y%m%d')
        actual_days = (end_date - start_date).days + 1
        print(f"实际跨越：{actual_days} 个自然日")
    
    # 测试用例2：包含多个字段的查询
    sql2 = """SELECT * FROM (
        SELECT trade_date, open_price, high_price, low_price, close_price 
        FROM stock_history 
        WHERE stock_code = '000858.SZ' 
        ORDER BY trade_date DESC 
        LIMIT 30
    ) ORDER BY trade_date ASC"""
    
    print("\n" + "="*70)
    print("\n📝 测试用例2：多字段查询（五粮液）")
    print("原始SQL:")
    print(sql2)
    
    processed_sql2 = preprocess_recent_month_sql(sql2)
    print("\n处理后SQL:")
    print(processed_sql2)
    
    df2 = pd.read_sql(text(processed_sql2), engine)
    print(f"\n✅ 返回 {len(df2)} 条记录")
    if len(df2) > 0:
        print(f"日期范围：{df2['trade_date'].iloc[0]} 至 {df2['trade_date'].iloc[-1]}")
        
        start_date = datetime.strptime(str(df2['trade_date'].iloc[0]), '%Y%m%d')
        end_date = datetime.strptime(str(df2['trade_date'].iloc[-1]), '%Y%m%d')
        actual_days = (end_date - start_date).days + 1
        print(f"实际跨越：{actual_days} 个自然日")
    
    # 测试用例3：非"最近一个月"查询（不应被修改）
    sql3 = """SELECT trade_date, close_price 
    FROM stock_history 
    WHERE stock_code = '600519.SH' 
      AND trade_date >= '20240101' 
    ORDER BY trade_date ASC"""
    
    print("\n" + "="*70)
    print("\n📝 测试用例3：具体日期范围查询（不应被修改）")
    print("原始SQL:")
    print(sql3)
    
    processed_sql3 = preprocess_recent_month_sql(sql3)
    print("\n处理后SQL:")
    print(processed_sql3)
    
    if sql3 == processed_sql3:
        print("✅ SQL未被修改（符合预期）")
    else:
        print("❌ SQL被意外修改（不符合预期）")
    
    print("\n" + "="*70)
    print("总结：")
    print("="*70)
    print("1. ✅ 检测到 LIMIT 30 时，自动转换为自然月30天查询")
    print("2. ✅ 在应用层计算起始日期（最新日期 - 30天）")
    print("3. ✅ 替换为具体的日期范围查询（WHERE trade_date >= 'YYYYMMDD'）")
    print("4. ✅ 非LIMIT 30的查询不受影响")
    print("="*70)


if __name__ == '__main__':
    test_sql_preprocessing()
